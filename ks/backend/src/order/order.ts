...
export namespace OrderNS {
    ...
    export const Utils = {
        ...
        ReportData(array: Array<OrderNS.ViewOrder>, option: "basic" | "medium" | "good" | "luxury") {
            ...
            if (now === +0) {
                startWeek = startOfWeek(Date.now()).getTime() - ONE_DAY * 6;
            } else {
                startWeek = startOfWeek(Date.now()).getTime() + ONE_DAY;
            }
            for (let i = +1; i <= now; i++) {
                ...
                let count = +0;
                let total = +0;
                const result = array.filter((r: OrderNS.ViewOrder) => {
                    if (r.ctime >= startOfDay && r.ctime <= endOfDay) return r;
                })
                if (result.length === +0) {
                    obj[option].total.push(+0)
                    obj[option].amount.push(+0);
                } else {
                    count = result.length;
                    total = result.reduce((total, obj) => {
                        return total += obj.total
                    }, +0)
                    obj[option].total.push(total)
